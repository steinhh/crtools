...
class TestFsigma3EdgeCases:
    ...
    def test_fsigma3_uniform_array(self):
        """Test fsigma3 with uniform values returns zero sigma."""
...
class TestFsigma3Integration:
    ...
    def test_module_uniform_array(self):
        """Test with uniform array (sigma should be zero)."""
        input_arr = np.ones((5, 5, 5), dtype=np.float64)
        out = fsigma3(input_arr, 3, 3, 3, 1)
        assert np.allclose(out, np.zeros_like(input_arr))
    
    def test_module_high_variance_detection(self):
        """Test that high variance areas are detected."""
        # Create array with outlier
        input_arr = np.ones((5, 5, 5), dtype=np.float64)
        input_arr[2, 2, 2] = 100.0
        
        # Filter with exclude_center=0 (include outlier)
        out_with_center = fsigma3(input_arr, 3, 3, 3, 0)
        
        # Filter with exclude_center=1 (exclude outlier)
        out_without_center = fsigma3(input_arr, 3, 3, 3, 1)
    ...
    def test_module_nan_handling(self):
        ...
        # Result should not contain unexpected NaN
        assert not np.isnan(out[2, 2, 2])
    ...
    def test_module_window_sizes(self):
        ...
        # Test 1x1x1 window with center included (should be zero)
        out = fsigma3(input_arr, 1, 1, 1, 0)
        assert np.allclose(out, np.zeros_like(input_arr))
