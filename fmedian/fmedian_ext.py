"""
Wrapper that loads the compiled `fmedian_ext` extension from the local directory and
...
# Look for a compiled extension file in the current directory matching fmedian_ext*.so
_so_path = None
...
if _so_path is None:
    ...
    try:
        ...
        raise ImportError('Could not find compiled fmedian_ext extension (.so) in local directory and import failed') from e
else:
