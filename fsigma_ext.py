...
Prefer using the package-qualified import ``from fsigma import fsigma_ext``.
...
try:
    ...
    from fsigma import fsigma_ext as _real
except Exception:
    # Fallback: try to import a top-level compiled extension (legacy)
    try:
        import importlib

        _real = importlib.import_module('fsigma_ext')
    except Exception as e:  # re-raise as ImportError to show helpful message
        raise ImportError(
            "Could not import 'fsigma_ext'. Build the extension with: python setup.py build_ext --inplace"
        ) from e
